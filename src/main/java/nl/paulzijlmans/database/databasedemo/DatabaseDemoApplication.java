...
import nl.paulzijlmans.database.databasedemo.jdbc.PersonJdbcDao;
...
public class DatabaseDemoApplication implements CommandLineRunner {
  ...
  public void run(String... args) {
    ...
    logger.info("User(s) name Pieter -> {}", dao.findByName("Pieter"));
    logger.info("Deleting 10002 -> No of Rows Deleted - {}", dao.deleteById(10002));
    logger.info(
        "Inserting 10004 -> {}", dao.insert(new Person(10004, "Tara", "Berlin", new Date())));
    logger.info(
        "Updating 10003 -> {}", dao.update(new Person(10003, "Pieter", "Utrecht", new Date())));
    logger.info("All users -> {}", dao.findAll());
  }
}
